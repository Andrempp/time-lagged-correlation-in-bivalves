...
def impute_missing_values(df, sensorDict, imputationMethodMode, randomSeed):
    """
    Impute missing values using multiple methods across multipe timeseries

    :param pd.DataFrame df: Dataset to impute
    :param dict sensorDict: Dictionary of type: {'sensor1': ['imputeMethod', 'parameters']} where sensor 1 is the column name
    ...
    for sensor in sensorDict.keys():
        ...
        if (imputationMethodMode == 'default'):
            ...
            if (imputeMethod == 'Mean'):
                ...
                reconstructedData = random_forests(df.copy(), parameters,
                                                   sensor)  # so deixar quando a time serie é multi variada
...
def interpolation_method(df, parameters, sensor):
    df[sensor] = df[sensor].astype(float).interpolate(method=parameters[0])
...
def moving_average(df, parameters, sensor):
    #impute nan at the beginning
    for index, row in df.iterrows():
        if np.isnan(row[sensor]):
            df.loc[index, sensor] = df[sensor].mean()
        else:
            break

    while df[sensor].isna().any():
        #df[sensor] = df[sensor].rolling(window=parameters[0], min_periods=parameters[1], center=parameters[2]).mean()
        df["roll"] = df[sensor].rolling(window=parameters[0], min_periods=parameters[1], center=parameters[2]).mean()
        df['roll'].update(df[sensor])
        df[sensor] = df['roll']
        df.drop(columns=['roll'])
    return df
...
# imputes all variables at same time
...
def mtsdi(df, parameters, sensor):
    x = impy.em(df.values, loops=parameters[0])
    newDf = pd.DataFrame(x, columns=df.columns, index=df.index)
    return newDf
...
def mice(df, parameters, sensor):
    ...
    # connect to R
    command = ["Rscript", f"{path}imputation_methods.r"]
    df.to_csv(f'input_data.csv', index=False)
    args = ['mice'] + parameters
    args = [str(i) for i in args]
    # subprocess.call(" ".join(command + args), shell = True)
    # subprocess.call("ls functions", shell = True)
    try:
        outputR = subprocess.run(" ".join(command + args), shell=True, check=True, stderr=subprocess.DEVNULL)
    ...
    # outputR = subprocess.check_output(" ".join(command + args), universal_newlines=True)
    # outputR = subprocess.check_output("ls functions", universal_newlines=True)
...
def amelia(df, parameters, sensor):
    ...
    args = [str(i) for i in args]
    outputR = subprocess.run(" ".join(command + args), shell=True, check=True)
    #print(outputR)
    # outputR = subprocess.check_output(" ".join(command + args), universal_newlines=True)
